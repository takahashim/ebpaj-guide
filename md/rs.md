...
スタイルシートで -epub-text-orientation を用い、文字の向きを変更した場合でも、em サイズは不変と
する。
...
このとき、要素のページ進行方向でない側（縦組みなら高さ）には、要素にページフィット用の指定を
...
カバーページには見開き画面の中央に表示される指定をするが、後につづく見開き配置が崩れないのであ
